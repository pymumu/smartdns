...
const char *HTTP2_TEST_ROOT_CERT_FILE = "/tmp/smartdns-http2-test-root-ca.pem";
const char *HTTP2_TEST_ROOT_KEY_FILE = "/tmp/smartdns-http2-test-root-ca.key";
...
bool EnsureHTTP2TestCert()
{
	...
	return generate_cert_key(HTTP2_TEST_KEY_FILE, HTTP2_TEST_CERT_FILE, HTTP2_TEST_ROOT_KEY_FILE,
							 HTTP2_TEST_ROOT_CERT_FILE, "DNS:smartdns,IP:127.0.0.1", 1) == 0;
}
...
TEST(HTTP2, DownstreamDohServerManyConnectionsManyConcurrentStreams)
{
	...
	smartdns::Server server;
	server.Start(R"""(bind-https [::]:60053 -alpn h2
address /h2-concurrent.test/1.2.3.4
log-level error
)""");
...
TEST(HTTP2, Http2UpstreamMaxConcurrentStreamsDoesNotBlockCompletedResponses)
{
	Defer
	{
		...
		unlink(HTTP2_TEST_ROOT_CERT_FILE);
		unlink(HTTP2_TEST_ROOT_KEY_FILE);
	};
