  ...
  public:
	...
	void Stop()
	{
		...
		for (auto &t : worker_threads_) {
			if (t.joinable()) {
				t.detach();
			} // Allow workers to die naturally
		}
	}
...
static bool perform_socks5_handshake(int fd)
{
	...
	if (send(fd, greet, sizeof(greet), 0) != sizeof(greet)) {
		return false;
	}

	unsigned char resp[2];
	if (recv(fd, resp, sizeof(resp), 0) != 2) {
		return false;
	}
	...
	unsigned char req[] = {0x05, 0x01, 0x00, 0x01, 0x7F, 0x00, 0x00, 0x01, 0xD2, 0xF0};
	if (send(fd, req, sizeof(req), 0) != sizeof(req)) {
		return false;
	}

	unsigned char resp2[1024];
	if (recv(fd, resp2, sizeof(resp2), 0) < 4) {
		return false;
	}
	return true;
}
