...
private:
    ...
    void HandleClient(int fd) {
        ...
        char ver;
        if (ReadN(fd, &ver, 1) <= 0) { close(fd); return; }
        if (ver != 0x05) { close(fd); return; }
        
        char nmethods;
        if (ReadN(fd, &nmethods, 1) <= 0) { close(fd); return; }
        
        if (ReadN(fd, buffer, (unsigned char)nmethods) <= 0) { close(fd); return; }
        ...
        if (ReadN(fd, buffer, 4) <= 0) { close(fd); return; }
        
        // Read Address
        if (buffer[3] == 0x01) { // IPv4
            if (ReadN(fd, buffer + 4, 4 + 2) <= 0) { close(fd); return; } // IP(4) + Port(2)
        } else if (buffer[3] == 0x03) { // Domain
             if (ReadN(fd, buffer + 4, 1) <= 0) { close(fd); return; }
             int dom_len = buffer[4];
             if (ReadN(fd, buffer + 5, dom_len + 2) <= 0) { close(fd); return; }
        } else if (buffer[3] == 0x04) { // IPv6
             if (ReadN(fd, buffer + 4, 16 + 2) <= 0) { close(fd); return; }
        }
...
log-level error)""");
    ...
    // forward-server listens on a random free port (let's pick one, or use 0 and parse? smartdns server usually needs fixed port or we need to parse logs/output, but here we can just pick a likely free port or rely on Server logic if it supported dynamic ports properly in config strings, but hardcoded is easier for test)
    // We'll use 60062 as intended in previous code block but correctly configured.
...
proxy-server socks5://127.0.0.1:11062 -name myproxy
    ...
    conf += "forward-server 127.0.0.1:" + std::to_string(forward_port) + " -target 127.0.0.1:" + std::to_string(echo_port) + " -proxy myproxy\n";
        ...
        threads.emplace_back([&, forward_port]() {
            ...
            if (n > 0) {
                ...
                if (strcmp(buf, msg) == 0) {
                    ...
                    // The requirement was "latency < 20ms". With 20 threads, we check if it generally holds or check the average/max.
                    // For strict test, we check if it is reasonable.
