  ...
  private:
	...
	void HandleClient(int client_fd)
	{
		...
		if (buf[1] == 0x01) { // CONNECT (Not supported for this test)
			...
			// Client -> SOCKS_UDP_FD -> Extract Header -> Send to Upstream -> Recv Reply -> Add Header -> Send to
			// Client Since this is a simple mock, we only handle ONE packet or loop. Client address is unknown until we
			// receive 1st packet.
			...
			while (running_) {
				...
				if (n > 10 && buf[0] == 0 && buf[1] == 0 && buf[2] == 0) {
					...
					if (rn > 0) {
						...
						sendto(udp_fd, resp_buf, 10 + rn, 0, (struct sockaddr *)&last_client_addr,
							   sizeof(last_client_addr));
					}
				}
			}
			close(udp_fd);
